...
<div class="mb-5 flex flex-col md:flex-row justify-between items-center md:items-start">
    <div class="flex items-center">
        <h2 class="text-3xl font-extrabold text-gray-200 mt-2 md:mt-0 ml-2 transform hover:scale-105 transition-transform duration-300 ease-in-out">Modifier Matière</h2>
    </div>
...
<div class="bg-white shadow-md rounded-lg p-6">
    <form method="POST" action="{{ route('matiere.update', $matiere->CodeMatiere) }}">
        @csrf
        @method('PUT')
        <div class="mb-4">
            <label for="LibelleMatiere" class="block text-sm font-medium text-gray-700">Libellé Matière</label>
            <input type="text" name="LibelleMatiere" placeholder="Libellé Matière" class="mt-1 p-2 border rounded-md w-full" value="{{ $matiere->LibelleMatiere }}" required>
        </div>
