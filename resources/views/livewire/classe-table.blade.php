<div>
    <div class="mb-5 flex flex-col md:flex-row justify-between items-center md:items-start">
        <div class="flex items-center">
            <h2 class="text-3xl font-extrabold text-gray-200 mt-2 md:mt-0 ml-2 transform hover:scale-105 transition-transform duration-300 ease-in-out">Classes</h2>
        </div>
        <div class="flex flex-col md:flex-row items-center space-y-2 md:space-y-0 md:space-x-4">
            ...
            <div class="relative">
                <select class="border rounded-full py-2 px-4 pl-10 focus:outline-none focus:ring focus:border-to-purple-300" wire:model.live="selectedClasse">
                    ...
                    @foreach($allClasses as $classeId => $classeName)
                        <option value="{{ $classeId }}">{{ $classeName }}</option>
                    @endforeach
                </select>
            </div>
            <a href="{{ route('classe.create') }}" class="px-4 py-2 bg-gradient-to-r from-indigo-500 to-purple-500 text-white rounded-md hover:bg-gradient-to-r hover:from-indigo-600 hover:to-purple-600 focus:outline-none focus:bg-gradient-to-r focus:from-indigo-600 focus:to-purple-600">
        ...
        <table class="min-w-max w-full table-auto">
            ...
            <tbody class="text-gray-600 text-sm">
                ...
                <tr class="border-b border-gray-200 hover:bg-gray-100">
                    <td class="pl-5 text-purple-600">{{ $classe->NomClasse }}</td>
                    <td class="py-3 px-6 text-left">
                        <ul>
                            @foreach($classe->matieres as $matiere)
                            <li>{{ $matiere->LibelleMatiere }}</li>
                            @endforeach
                        </ul>
                    </td>
                    <td class="py-3 px-6 text-center">
                        <div class="flex item-center justify-center">
                            <a href="{{ route('classe.show', $classe->IdClasse) }}" class="w-4 mr-2 transform hover:text-purple-500 hover:scale-110 cursor-pointer">
                                <i class="fas fa-eye"></i>
                            </a>
                            <a href="{{ route('classe.edit', $classe->IdClasse) }}" class="w-4 mr-2 transform hover:text-purple-500 hover:scale-110 cursor-pointer">
