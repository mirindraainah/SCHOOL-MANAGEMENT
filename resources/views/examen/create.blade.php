...
<div class="mb-5 flex flex-col md:flex-row justify-between items-center md:items-start">
    <div class="flex items-center">
        <h2 class="text-3xl font-extrabold text-gray-200 mt-2 md:mt-0 ml-2 transform hover:scale-105 transition-transform duration-300 ease-in-out">Créer un Examen</h2>
    </div>
</div>

<div class="bg-white shadow-md rounded-lg p-6">
    <form action="{{ route('examen.store') }}" method="post" class="grid grid-cols-1 md:grid-cols-2 gap-4">
        @csrf
        <div class="mb-4">
            <label for="LibelleExamen" class="block text-sm font-medium text-gray-700">Libellé de l'Examen</label>
            <input type="text" id="LibelleExamen" name="LibelleExamen" class="form-input mt-1 p-2 border rounded-md w-full">
        </div>
